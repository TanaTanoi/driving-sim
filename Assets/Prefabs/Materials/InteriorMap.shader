	...
	SubShader
	{
		...
		Pass{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				/* Furniture stuff, doesn't look great due to the incorrect UVs
				float3 furInter;
				...
				/* If sim is running, use this */
				//c += abs(sin(((_SimTime + 12 * noiseColour.b) + (noiseColour.b * 20)) * noiseColour.b * 0.5);
				/* If sim is still, use this */
				c += abs(sin(((_Time.y + 12) * 0.1 * noiseColour.b) + (noiseColour.b * 20)) * noiseColour.b * 0.5);
				...
				float4 windowCol = tex2D(_Window, i.pos.xy / _FloorHeight) * max(0, dot(i.normal.xyz, _WorldSpaceLightPos0.xyz));
